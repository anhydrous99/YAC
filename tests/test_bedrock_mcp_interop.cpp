...
constexpr std::string_view kTool38 = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
...
TEST_CASE("mcp_60char_sanitized_name_is_60_chars", "[bedrock_mcp_interop]") {
  const std::string name = yac::mcp::SanitizeMcpToolName(kServer16, kTool38);
...
TEST_CASE("mcp_60char_tool_in_translate_tool_definitions",
          "[bedrock_mcp_interop]") {
  const std::string name = yac::mcp::SanitizeMcpToolName(kServer16, kTool38);
  std::vector<ToolDefinition> tools = {{.name = name,
...
TEST_CASE("mcp_60char_tool_use_id_round_trips", "[bedrock_mcp_interop]") {
  const std::string name = yac::mcp::SanitizeMcpToolName(kServer16, kTool38);
  auto req = TranslateToolUseToYac("call-mcp-001", name, "{}");
  REQUIRE(req.id == "call-mcp-001");
}

TEST_CASE("mcp_60char_tool_use_name_round_trips", "[bedrock_mcp_interop]") {
  const std::string name = yac::mcp::SanitizeMcpToolName(kServer16, kTool38);
  auto req = TranslateToolUseToYac("call-mcp-002", name, R"({"x":"y"})");
