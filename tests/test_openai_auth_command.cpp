...
TEST_CASE("status uses inline config before env for compatible provider",
          "[openai_auth_command]") {
...
TEST_CASE("status ignores stored auth when compatible provider uses env",
          "[openai_auth_command]") {
  TempDir tmp("yac_test_openai_auth_command_compatible_env_status");
  const auto settings_path = tmp.Path() / "settings.toml";
  WriteFile(settings_path,
            "[provider]\n"
            "id = \"openai-compatible\"\n"
  ...
  const auto store = MakeStore(backend);
  static_cast<void>(
      store->Save(yac::provider::OpenAiApiKeyAuth{.key = "stored-secret"}));
  ScopedEnvVar env("OPENAI_API_KEY", "env-secret");
  ...
  REQUIRE(summary.configured_provider == "openai-compatible");
  REQUIRE(summary.stored_credential == std::optional<std::string>{"api"});
  REQUIRE(summary.effective_auth ==
          std::optional<std::string>{"api (env OPENAI_API_KEY)"});
  REQUIRE(AllWarnings(summary).find("shadowed") == std::string::npos);
  REQUIRE(AllWarnings(summary).find("stored-secret") == std::string::npos);
  REQUIRE(AllWarnings(summary).find("env-secret") == std::string::npos);
}

TEST_CASE("status reports env shadowing stored auth for openai provider",
          "[openai_auth_command]") {
  TempDir tmp("yac_test_openai_auth_command_openai_env_status");
  const auto settings_path = tmp.Path() / "settings.toml";
  WriteFile(settings_path,
            "[provider]\n"
            "id = \"openai\"\n"
  ...
  REQUIRE(summary.configured_provider == "openai");
  REQUIRE(summary.stored_credential == std::optional<std::string>{"api"});
  REQUIRE(summary.effective_auth ==
          std::optional<std::string>{"api (env OPENAI_API_KEY)"});
  REQUIRE(AllWarnings(summary).find("stored credential is shadowed") !=
          std::string::npos);
  REQUIRE(AllWarnings(summary).find("stored-secret") == std::string::npos);
  REQUIRE(AllWarnings(summary).find("env-secret") == std::string::npos);
  REQUIRE(AllWarnings(summary).find("config-secret") == std::string::npos);
